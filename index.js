...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    let a = Math.random() * characters.length;
    let b = Math.floor(a);
    let c = characters[b];
    password += c;
  }
...
function calculateCircleArea(radius) {
  ...
  if (typeof radius !== "number") {
    console.log(`Помилка: значення має бути числом`);
    return null;
  }
...
function calculateHypotenuse(a, b) {
  ...
  let lengthHypotenuse = Math.sqrt(Math.pow(a, 2) + Math.pow(b, 2));
...
function estimateInvestment(principal, interestRate, years) {
  ...
  let investmen = Math.round(principal * Math.pow(1 + interestRate, years));
  return investmen;
}
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  const totalPrice = products.reduce((elem) => elem + products.price, 0);
  return Math.fround(totalPrice) === Math.floor(maxPrice);
}
